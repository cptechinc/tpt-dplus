<?php
	if ($input->get->code) {
		...
		$existing_countries_array = array();
		foreach ($existing_countries as $country) {
			array_push($existing_countries_array, $country);
		}

        $countries = CountryCodesQuery::create()->find();
		$tariff = $module_codetable->get_code($code);

		$page->body .= $config->twig->render("code-tables/min/$page->codetable/form.twig", ['page' => $page, 'table' => $page->codetable, 'tariff_country_count' => $tariff_country_count, 'existing_countries' => $existing_countries_array, 'countries' => $countries, 'tariff' => $tariff]);
